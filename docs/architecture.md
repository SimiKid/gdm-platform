...
- **Condition management** — CRUD for experimental conditions, seeded on first startup with three arms (`baseline`, `public-llm`, `private-llm`: a silent baseline plus public/private nudge delivery, both nudging arms using rule+LLM detection).
...
- **Condition assignment** — if the join request includes a `conditionId` (e.g. from a pilot link `?conditionId=public-llm`), that condition is used. Otherwise, the system picks the least-completed active condition (balanced assignment). Assignment happens server-side.
...
- **LLM classifier** — in the nudging arms (`llmMode: "active"`), an Anthropic-backed contribution classifier (`src/classifier/`) scores message meaningfulness; the scores feed the composite dominance metric and invite grace, but never trigger nudges on their own.
