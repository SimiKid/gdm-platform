...
import { filterResearchSessions } from "./filter";
import {
  contributionScores,
  countWords,
  gini,
...
export class ReportsService {
  ...
  private async sessions(conditionIds: string[]): Promise<Session[]> {
    return filterResearchSessions(await this.store.allSessions(), conditionIds);
  }

  // ── participants ─────────────────────────────────────────────────

  async exportParticipants(conditionIds: string[] = []) {
    return {
      generatedAt: new Date().toISOString(),
      participants: (await this.sessions(conditionIds)).flatMap((session) =>
  ...
  async exportParticipantsCsv(conditionIds: string[] = []): Promise<string> {
    const { participants } = await this.exportParticipants(conditionIds);
    return toCsv([
      [
  ...
  async exportSessionsAnalysis(conditionIds: string[] = []) {
    return {
      generatedAt: new Date().toISOString(),
      sessions: (await this.sessions(conditionIds)).map(sessionRow),
    };
  }

  async exportSessionsAnalysisCsv(conditionIds: string[] = []): Promise<string> {
    const { sessions } = await this.exportSessionsAnalysis(conditionIds);
    return toCsv([
      [
        "session_pseudonym",
        "condition_id",
  ...
  async exportWindows(conditionIds: string[] = []) {
    return {
      generatedAt: new Date().toISOString(),
      windows: (await this.sessions(conditionIds)).flatMap((session) =>
   ...
   */
  async exportWindowsCsv(conditionIds: string[] = []): Promise<string> {
    const sessions = await this.sessions(conditionIds);
    const rows: string[][] = [];
  ...
  async exportResearchMessagesCsv(conditionIds: string[] = []): Promise<string> {
    const sessions = await this.sessions(conditionIds);
    return toCsv([
      [
        "session_pseudonym",
  ...
  async exportLinkageCsv(conditionIds: string[] = []): Promise<string> {
    const sessions = await this.sessions(conditionIds);
    return toCsv([
      [
        "participant_pseudonym",
  ...
  async summary(conditionIds: string[] = []): Promise<ReportsSummaryResponse> {
    const sessions = await this.sessions(conditionIds);
    const conditions = (await this.store.listConditions()).filter(
      (condition) =>
        !condition.id.startsWith("e2e-") &&
        (conditionIds.length === 0 || conditionIds.includes(condition.id)),
    );
  ...
  async bundleZip(conditionIds: string[] = []): Promise<Buffer> {
    const [participants, sessionsAnalysis, windows, messages] =
      await Promise.all([
        this.exportParticipantsCsv(conditionIds),
        this.exportSessionsAnalysisCsv(conditionIds),
        this.exportWindowsCsv(conditionIds),
        this.exportResearchMessagesCsv(conditionIds),
      ]);
...
All exports accept \`?conditionIds=a,b,c\` to restrict to specific arms.
Sessions from \`e2e-…\` test conditions are always excluded.
